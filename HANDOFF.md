...
**CSS/JS 缓存刷新**：每次改 `script.js` 或 `styles.css` 后，把 `index.html` 里对应的 `?v=` 版本号 +1。当前：`styles.css?v=17`、`script.js?v=13`。
...
- 商品详情弹窗（点分类卡片弹出的那个）在"一排天生只能放下 1 个商品"的窄屏下（用 `modalProducts.clientWidth` 按 `auto-fill, minmax(200px,...)` 的公式反算，不是写死的媒体查询断点），标题下方会出现一个"每排显示 1/2/3 个"的小切换按钮（简单的框格图标，选中态高亮），方便小屏幕用户一次多看几件商品再点进详情；电脑/平板宽度只要天生能放下 2 个以上就不会出现这个按钮，也不受影响。选择会记到 `localStorage`（key: `modalGridCols`），下次打开任何分类都记得上次选的排列方式
...
- [ ] 前台网页整体布局和设计（持续渐进调整）
- [ ] 会员/学员/上课记录管理（还没想好方案）
- [ ] 后台与前台无缝衔接：管理员身份下能直接在主页和后台之间切换，不用靠两个不同链接分别打开
