...
- **admin.html 不再有自己的登录表单**：唯一的登录入口是首页。未登录状态访问 admin.html 只会看到一个
  "请先在首页登录管理员账号" + 跳首页的链接，跟 member.html 的处理方式一致。登录成功后"踢掉其他设备"
  的 `signOut({scope:'others'})` 调用统一放在落地页确认完身份之后再执行（不能紧跟在登录成功后立刻做，
  紧跟着跳转会把刚建好的本机登录状态一起冲掉，踩过这个坑），且只在 `sessionStorage` 有
  `justLoggedIn` 标记时执行一次，避免每次刷新页面都误伤正在使用的其他设备
